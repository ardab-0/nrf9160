...
import queue
import os
from single_measurement.fsm import Controller
import json
...
class Serial_Communication:
    ...
    def evaluate(self, response, q):
        ...
        if response == "OK" and (self.controller.state != "wait_measurement_result"
                                 and self.controller.state != "measure"):
            self.controller.ok()
        elif response == "OK" and (self.controller.state == "wait_measurement_result"):
            return "", measurement_batch
        elif response.find("%NCELLMEAS: 0") >= 0 and (self.controller.state == "wait_measurement_result"):
